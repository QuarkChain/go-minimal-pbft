...
func (s *Server) AddConnectionHandler(reqFunc, goodByeFunc func(ctx context.Context, id peer.ID) error) {
	...
	s.Host.Network().Notify(&network.NotifyBundle{
		ConnectedF: func(net network.Network, conn network.Conn) {
			...
			disconnectFromPeer := func() {
				s.peers.SetConnectionState(remotePeer, PeerDisconnecting)
				...
				s.peers.SetConnectionState(remotePeer, PeerDisconnected)
			}
			...
			go func() {
				...
				if err == nil && (peerConnectionState == PeerConnected || peerConnectionState == PeerConnecting) {
				...
				if s.peers.IsBad(remotePeer) {
					log.Trace("Ignoring connection request", "reason", "bad peer")
					disconnectFromPeer()
					return
				}
				validPeerConnection := func() {
					s.peers.SetConnectionState(conn.RemotePeer(), PeerConnected)
				...
				s.peers.SetConnectionState(conn.RemotePeer(), PeerConnecting)
				if err := reqFunc(context.TODO(), conn.RemotePeer()); err != nil && err != io.EOF {
					log.Trace("Handshake failed")
					disconnectFromPeer()
					return
				}
				validPeerConnection()
...
func (s *Server) AddDisconnectionHandler(handler func(ctx context.Context, id peer.ID) error) {
	s.Host.Network().Notify(&network.NotifyBundle{
		DisconnectedF: func(net network.Network, conn network.Conn) {
			...
			go func() {
				...
				if err != nil {
					...
					priorState = PeerDisconnected
				}
				s.peers.SetConnectionState(conn.RemotePeer(), PeerDisconnecting)
				...
				s.peers.SetConnectionState(conn.RemotePeer(), PeerDisconnected)
				// Only log disconnections if we were fully connected.
				if priorState == PeerConnected {
					log.Debug("Peer disconnected", "activePeers", len(s.peers.Active()))
				}
			}()
		},
	})
